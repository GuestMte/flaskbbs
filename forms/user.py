...
class RegisterForm(BaseForm):
  ...
  def validate_captcha(self,field):
    ...
    cache_captcha = cache.get(email)#获取邮箱
    if not cache_captcha or captcha != cache_captcha:#检查缓存中是否存在与邮箱地址对应的验证码，并且用户输入的验证码是否与缓存中的验证码匹配
...
class LoginForm(BaseForm):
  email = StringField(validators=[Email(message="请输入正确格式的邮箱！")])
  password = StringField(validators=[Length(min=6, max=20, message="请输入正确长度的密码！")])
  remember = BooleanField()#是否点击了记住我
